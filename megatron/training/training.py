...
def get_megatron_ddp_config(args: argparse.Namespace) -> DistributedDataParallelConfig:
    ...
    if getattr(args, "use_torch_fsdp2", False):
        ...
        if args.use_megatron_fsdp and args.cuda_graph_impl != "none":
            # When using CUDA graphs, Megatron-FSDP should not AG parameters during
            # start_param_sync(), which is called during the DistOpt.step(). This
            # causes an error when we wait() on a CUDA kernel launched in a stream
            # beyond the scope of the FWD-BWD or optimizer CUDA graph capture.
...
def train_step(forward_step_func, data_iterator, model, optimizer, opt_param_scheduler, config, forward_backward_func, iteration=None):
    ...
    # Convert the zero grad counter from Tensor to int.
    num_zeros_in_grad = num_zeros_in_grad.item()
